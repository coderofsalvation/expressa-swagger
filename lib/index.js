...
module.exports = function(opts){ 
          ...
          "parameters": [
            {
              "in": "body",
              "name": "payload",
              "description": "pass email & password as credentials",
              "required": true,
              "schema": {
                "type": "object",
                "properties":{
  ...
  var generateEndpoints = function(collectionname){
    ...
    path[method].tags = [collectionname]
    path[method].parameters = [ parameterId ] 
    path[method].summary = "Overwrites a specific "+collectionname+" item. Use POST "+collectionpath_id+"/update to only modify particular fields"
    ...
    path[method].tags = [collectionname]
    path[method].parameters = [ parameterId ] 
    path[method].summary = "Deletes a specific "+collectionname+" item"
  }

  expressa.swagger = {
    addEndpoint: function(method, path, data){
      swagger.paths[path] = swagger.paths[path] || {}
			swagger.paths[path][method] = data
    }
  }
  
  app.use( '/api/swagger', function(req, res, next){
    for( var collectionname in expressa.db ) generateEndpoints(collectionname)
